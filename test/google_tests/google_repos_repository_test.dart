...
class MockService extends Mock implements GoogleReposServices {}
...
void main() {
  ...
  GoogleReposRepository repository;

  setUp(() {
    service = MockService();
    repository = GoogleReposRepository(service);
  });

  test('when repo  is called should call service', () async {
    when(service.fetchGoogleRepos(any)).thenAnswer((_) async => mockedResponse);
    await repository.fetchGoogleRepos(0);
    verify(service.fetchGoogleRepos(0));
  });

  test('when repo is called and service throw then throw', () async {
    when(service.fetchGoogleRepos(0)).thenThrow(FailFetchGoogleRepos());
    expect(() async => await repository.fetchGoogleRepos(0),
